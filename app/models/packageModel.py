...
from sqlalchemy import Date, Float, ForeignKey, Integer, Numeric, String, Enum as SQLEnum
...
class Package(Base, TimestampMixin):
    ...
    cooperative: Mapped["User"] = relationship("User", foreign_keys=[cooperative_id])
    ...
    investments: Mapped[List["Investment"]] = relationship(
        "Investment", back_populates="package", cascade="all, delete-orphan"
    )
...
class PackageFarmer(Base, TimestampMixin):
    ...
    allocated_amount: Mapped[float] = mapped_column(Numeric(12, 2), default=0.0, nullable=False)
    ...
    package: Mapped["Package"] = relationship("Package", back_populates="package_farmers")
...
class Investment(Base, TimestampMixin):
    ...
    payback_due_date: Mapped[date] = mapped_column(Date, nullable=False)
    package: Mapped["Package"] = relationship("Package", back_populates="investments")
